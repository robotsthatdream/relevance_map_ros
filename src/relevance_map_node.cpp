...
void relevance_map_node::_add_new_sample(uint32_t sv_lbl,int label){
    if(_method == "nnmap"){
        for(auto& classifier : _nnmap_class){
//            ROS_INFO_STREAM(classifier.first);
            classifier.second.add(_soi.get_feature(sv_lbl,classifier.first),label);
...
//            ROS_INFO_STREAM("BABBLING_NODE : size of dataset : " << classifier.second.dataset_size());
        ...
        _mcs.add(_soi.get_features(sv_lbl),label);
    }
}
